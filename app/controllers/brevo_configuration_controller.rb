...
class BrevoConfigurationController < ApplicationController
  ...
  def import_categories
    begin
      # Start background job
      job_id = SecureRandom.uuid
      job = CategoryImportJob.perform_later(@company.id, job_id)
      
      respond_to do |format|
        format.html { redirect_to brevo_configuration_path, notice: "Category import started." }
        format.json { render json: { success: true, message: "Category import started", job_id: job_id } }
      end
    rescue => e
      Rails.logger.error "Error starting category import: #{e.message}"
      respond_to do |format|
