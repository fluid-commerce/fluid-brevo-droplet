...
module BrevoHelper
  ...
  def create_brevo_campaign(company, name, subject, html_content, recipients = {})
    ...
    campaign_service = Brevo::CampaignService.new(company.integration_setting.credentials.dig('brevo', 'api_key'))
  ...
  def send_brevo_campaign(company, campaign_id)
    ...
    campaign_service = Brevo::CampaignService.new(company.integration_setting.credentials.dig('brevo', 'api_key'))
    campaign_service.send_campaign(campaign_id: campaign_id)
  end
  ...
  def create_brevo_webhook(company, url, events = ['sent', 'delivered', 'opened', 'clicked'])
    ...
    webhook_service = Brevo::WebhookService.new(company.integration_setting.credentials.dig('brevo', 'api_key'))
  ...
  def get_brevo_campaign_report(company, campaign_id)
    ...
    campaign_service = Brevo::CampaignService.new(company.integration_setting.credentials.dig('brevo', 'api_key'))
    campaign_service.get_campaign_report(campaign_id: campaign_id)
  end
  ...
  def test_brevo_connection(company)
    ...
    config_service = Brevo::ConfigurationService.new(company.integration_setting.credentials.dig('brevo', 'api_key'))
  ...
  def sync_fluid_customers(company, customers, list_id = nil)
    ...
    sync_service = Brevo::FluidContactSyncService.new(company.integration_setting.credentials.dig('brevo', 'api_key'))
